...
async def _live_run_for_venue(
    ...
    *,
    ...
    async def _close_adapter_only(target: ExchangeAdapter) -> None:
        ...
        if callable(close_cb):
            try:
                ...
                pass
        maybe_ex = getattr(target, "ex", None)
        if maybe_ex is not None and hasattr(maybe_ex, "close"):
            try:
                res = maybe_ex.close()
                if inspect.isawaitable(res):
                    await res
    ...
    def _filter_triangles_for_adapter(
        current_adapter: ExchangeAdapter,
        raw_tris: list[Triangle],
    ) -> tuple[list[Triangle], list[tuple[Triangle, list[str]]]]:
        try:
            markets = current_adapter.load_markets()
        except Exception:
            return list(raw_tris), []

        filtered: list[Triangle] = []
        missing_local: list[tuple[Triangle, list[str]]] = []
        is_alpaca = current_adapter.name().lower() == "alpaca"
        ...
        for tri in raw_tris:
            ...
            unsupported = [leg for leg in legs if not _supported(leg)]
            if unsupported:
                missing_local.append((tri, unsupported))
            else:
                filtered.append(tri)

        return filtered, missing_local
    ...
    prefer_native = bool(getattr(settings, "alpaca_prefer_native", True))
    force_native_ccxt = (
        str(getattr(settings, "alpaca_use_ccxt", "")).strip().lower() in force_ccxt
    )

    for attempt in (1, 2):
        triangles, missing = _filter_triangles_for_adapter(
            adapter, configured_triangles
        )
        if triangles or venue.lower() != "alpaca":
            break
        if attempt == 1 and prefer_native and not force_native_ccxt:
            log.info(
                "live@%s native Alpaca adapter lacks required markets; retrying with CCXTAdapter",
                venue,
            )
            await _close_adapter_only(adapter)
    ...
    # triangles already filtered; missing captured in loop above
    if not triangles:
