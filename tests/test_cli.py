...
def test_config_discover_writes_env(tmp_path, monkeypatch):
    """`config:discover` should persist triangles to the requested env file."""

    env_file = tmp_path / ".env"
    env_file.write_text(
        "FOO=1\nTRIANGLES_BY_VENUE={'other': [['AAA','BBB','CCC']]}\nBAR=2\n",
        encoding="utf-8",
    )

    class DummyDiscoverAdapter:
        def __init__(self) -> None:
    ...
    assert result.exit_code == 0
    assert adapter.load_markets_calls == 1

    env_text = env_file.read_text(encoding="utf-8")
    assert "FOO=1" in env_text
    assert "BAR=2" in env_text
    assert "# Auto-generated by `python -m arbit.cli config:discover`" in env_text

    assignment_line = next(
        line for line in env_text.splitlines() if line.startswith("TRIANGLES_BY_VENUE=")
    )
    parsed = config_cmds._parse_triangles_env(assignment_line.split("=", 1)[1])
    assert parsed["paper"] == [["ETH/USDT", "ETH/BTC", "BTC/USDT"]]
    assert parsed["other"] == [["AAA", "BBB", "CCC"]]
